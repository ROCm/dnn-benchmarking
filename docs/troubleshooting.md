...
| `--perf` | a **runnable** `perf`, and either `/proc/sys/kernel/perf_event_paranoid <= 1` or `CAP_PERFMON`/`CAP_SYS_ADMIN` for kernel events | Host, not the venv: `apt install linux-tools-$(uname -r)`. Distro `perf` is a wrapper that exits 2 when no linux-tools matches the running kernel — if a real binary is already installed for another version, put it first on `PATH` (`/usr/lib/linux-tools-<ver>/perf`). The paranoid sysctl is not namespaced, so a container reads the host's value; a root/privileged container bypasses it by capability and still records `cycles:k` |
